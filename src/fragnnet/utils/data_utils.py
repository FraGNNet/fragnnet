...
from fragnnet.utils.formula_utils import ELECTRON_MASS, MASS, NEUTRON_MASS
from fragnnet.utils.misc_utils import none_or_nan, np_temp_seed
...
JOBLIB_BACKEND = "loky"
FRAGNNET_JOBLIB_N_JOBS = os.environ.get("FRAGNNET_JOBLIB_N_JOBS", None)
JOBLIB_N_JOBS = (
    joblib.cpu_count() if FRAGNNET_JOBLIB_N_JOBS is None else int(FRAGNNET_JOBLIB_N_JOBS)
)
JOBLIB_TIMEOUT = 10800  # default is 300 seconds for "loky"
if FRAGNNET_JOBLIB_N_JOBS is not None:
    print(f"Using FRAGNNET_JOBLIB_N_JOBS={FRAGNNET_JOBLIB_N_JOBS} for joblib n_jobs")


def rdkit_import(*module_strs: str) -> tuple:
...
def randomize_smiles(smiles, rseed, isomeric=False, kekule=False):
    ...
    # np_temp_seed now yields a local RNG (numpy.random.Generator)
    with np_temp_seed(rseed) as rng:
        # use the generator's in-place shuffle to avoid touching global state
        rng.shuffle(ans)
...
def mol_from_inchi(
    inchi: str, ml_standardize: bool = True, canonicalize_tautomers: bool = True
) -> "Chem.rdchem.Mol":
    """return a rdkit mol from inchi str

    Args:
            inchi (str): inchi string, np.nan if fails
            ml_standardize (bool, optional): Whether to apply ML-specific standardization
                    that includes cleanup, parent fragment extraction,
                    and tautomer canonicalization. Defaults to True.
            canonicalize_tautomers (bool, optional): Whether to canonicalize tautomers
                    during standardization. Only used when ml_standardize=True.
                    Set to False to preserve the input tautomeric form. Defaults to True.
    Returns:
    ...
    try:
        ...
        if ml_standardize:
            mol = rdkit_ml_standardize(mol, canonicalize_tautomers=canonicalize_tautomers)
    except Exception:
        mol = np.nan
    if none_or_nan(mol):
        mol = np.nan
...
def rdkit_ml_standardize(
    mol: "Chem.rdchem.Mol", canonicalize_tautomers: bool = True
) -> "Chem.rdchem.Mol":
    """Standardize a molecule for consistent representation for spectra machine learning tasks.
    Do not directly copy this to other chemical machine leanring code base, check list of operations
    you want to perform first.
    This function performs several important standardization steps:
    1. Cleans up the molecule (removing Hs, disconnecting metals, etc.)
    2. Extracts the parent fragment (largest/most relevant piece)
    3. Canonicalizes tautomers to a standard form (if enabled)
    Adapted from https://bitsilla.com/blog/2021/06/standardizing-a-molecule-using-rdkit/

    Args:
            mol (rdkit.Chem.rdchem.Mol): Input molecule object
            canonicalize_tautomers (bool, optional): Whether to canonicalize tautomers.
                    Set to False to preserve the input tautomeric form. Defaults to True.

    Returns:
            rdkit.Chem.rdchem.Mol: Standardized molecule
    """
    # Import necessary RDKit modules
    modules = rdkit_import(
        "rdkit.Chem", "rdkit.Chem.MolStandardize", "rdkit.Chem.MolStandardize.rdMolStandardize"
    )
    rdMolStandardize = modules[-1]

    # Step 1: Normalize and reionize — skip MetalDisconnector (would strip covalent C–metal bonds)
    mol = rdMolStandardize.Normalize(mol)
    mol = rdMolStandardize.Reionize(mol)

    # Step 2: Get the parent fragment
    # - If the molecule contains multiple fragments (e.g., salt forms)
    # - Identifies and returns only the "parent" fragment
    # - Removes counterions, solvents, and other small fragments
    # mol = rdMolStandardize.FragmentParent(mol)

    # Step 3: Neutralize the molecule
    # - Attempts to remove or neutralize charges where chemically reasonable
    # - Important for consistent representation across different sources
    # - Helps with similarity calculations and model predictions
    # note that no attempt is made at reionization at this step  nor at ionization at some pH (rdkit has no pKa caculator)
    # the main aim to to represent all molecules from different sources  in a (single) standard way, for use in ML, catalogue, etc.
    # NOTE: This is NOT a very safe operation for MS/MS prediction
    # uncharger = rdMolStandardize.Uncharger() # Create uncharger object (no convenience method available)
    # mol = uncharger.uncharge(mol)

    # Step 4: Canonicalize tautomers
    # - Converts between different tautomeric forms to ensure consistent representation
    # - Selects a single canonical tautomer when multiple forms are possible
    # - Critical for ensuring the same compound is always represented identically
...
def mol_from_smiles(
    smiles: str, ml_standardize: bool = True, canonicalize_tautomers: bool = True
) -> "Chem.rdchem.Mol":
    """Convert a SMILES string to an RDKit molecule object

    This function creates an RDKit molecule from a SMILES string and optionally
    standardizes it using machine learning-specific standardization steps.

    Args:
            smiles (str): SMILES string representing the molecule
            ml_standardize (bool, optional): Whether to apply ML-specific standardization
                    that includes cleanup, parent fragment extraction, charge neutralization,
                    and tautomer canonicalization. Defaults to True.
            canonicalize_tautomers (bool, optional): Whether to canonicalize tautomers
                    during standardization. Only used when ml_standardize=True.
                    Set to False to preserve the input tautomeric form. Defaults to True.
    ...
    Returns:
            rdkit.Chem.rdchem.Mol: RDKit molecule object, or np.nan if conversion fails
    """
    modules = rdkit_import("rdkit.Chem")
    Chem = modules[0]
    try:
...
def mol_to_smiles(
    ...
    try:
        smiles = Chem.MolToSmiles(
            mol, canonical=canonical, isomericSmiles=isomericSmiles, kekuleSmiles=kekuleSmiles
        )
    except Exception:
        smiles = np.nan
...
def mol_to_formula(mol: "Chem.rdchem.Mol") -> str:
...
def mol_to_inchikey_s(mol: "Chem.rdchem.Mol") -> str:
    """get inchikey string from give mol object
...
def mol_to_mol_weight(mol, exact=True) -> float:
    """get mol weight from mol object
    Args:
...
def mol_to_charge(mol: "Chem.rdchem.Mol") -> int:
...
def mol_to_num_atoms(mol: "Chem.rdchem.Mol", heavy: bool = True) -> int | float:
...
def mol_to_num_bonds(mol: "Chem.rdchem.Mol", heavy: bool = True) -> int | float:
...
def mol_to_num_radicals(mol: "Chem.rdchem.Mol") -> int | float:
...
def check_neutral_charge(mol: "Chem.rdchem.Mol") -> bool:
...
def check_single_mol(mol: "Chem.rdchem.Mol") -> bool:
...
def inchi_to_smiles(inchi: str) -> str | float:
...
def make_morgan_fingerprint(
    mol: "Chem.rdchem.Mol", radius: int = 3, nbits: int = 2048, covert_to_np=True
) -> np.array:
    """Make Morgan fingerprint from mol object.

    Args:
        mol: RDKit mol object
        radius: Fingerprint radius (default: 3). Must be > 0.
        nbits: Number of bits in fingerprint (default: 2048). Must be > 0.
        covert_to_np: Whether to convert to numpy array (default: True)

    Returns:
        Fingerprint as numpy array or RDKit fingerprint object

    Raises:
        ValueError: If radius <= 0 or nbits <= 0
    """
    if radius <= 0:
        raise ValueError(f"morgan_radius must be positive integer, got {radius}")
    if nbits <= 0:
        raise ValueError(f"morgan_nbits must be positive integer, got {nbits}")

    modules = rdkit_import("rdkit.Chem.AllChem", "rdkit.DataStructs")
    AllChem = modules[0]
    ds = modules[1]
...
def make_rdkit_fingerprint(mol: "Chem.rdchem.Mol", nbits: int = 2048) -> np.array:
    """Make RDKit fingerprint from mol object.
    ...
    Raises:
        ValueError: If nbits <= 0
    """
    if nbits <= 0:
        raise ValueError(f"rdkit_nbits must be positive integer, got {nbits}")
...
def make_maccs_fingerprint(mol: "Chem.rdchem.Mol") -> np.ndarray:
...
def remove_stereochemistry(mol: "Chem.rdchem.Mol"):
    """_summary_
...
def par_apply(iterator, func, need_unpack=False, return_as_generator=False, n_jobs=JOBLIB_N_JOBS):
    """Apply a function to each item in an iterator in parallel.

    Args:
            iterator (_type_): _description_
            func (_type_): _description_
            need_unpack (bool, optional): _description_. Defaults to False.
            return_as_generator (bool, optional): _description_. Defaults to False.
            n_jobs (_type_, optional): _description_. Defaults to JOBLIB_N_JOBS.
...
def par_apply_series(series, func, tqdm_leave=True, use_tqdm: bool = True):
    """Apply a function to each item in a pandas Series in parallel.

    Args:
            series (pd.Series): The Series to process.
            func (callable): The function to apply to each item.
            tqdm_leave (bool, optional): Whether to leave the tqdm progress bar after completion. Defaults to True.

    Returns:
            _type_: _description_
    """
    series_iter = (
        tqdm(series.items(), desc=pformat(func), total=series.shape[0], leave=tqdm_leave)
        if use_tqdm
        else series.items()
    )
...
def seq_apply_series(series, func, tqdm_leave=True, use_tqdm: bool = True) -> pd.Series:
    """Apply a function to each item in a pandas Series in sequence.
    ...
    Returns:
            pd.Series: The results of applying the function to each item.
    """
    series_iter = (
        tqdm(series.items(), desc=pformat(func), total=series.shape[0], leave=tqdm_leave)
        if use_tqdm
...
def par_apply_df_rows(df, func, use_tqdm: bool = True):
    ...
    Returns:
            pd.Series: The results of applying the function to each row.
    """

    df_iter = (
        tqdm(df.iterrows(), desc=pformat(func), total=df.shape[0]) if use_tqdm else df.iterrows()
    )
    ...
    if isinstance(result_list[0], tuple):
        raise NotImplementedError("Use par_apply_df_rows_t for multiple return values")
    result_series = pd.Series(result_list, index=df.index)

    return result_series


def par_apply_df_rows_t(df, func, use_tqdm: bool = True) -> tuple[pd.Series, ...]:
    """Apply a function to each row of a DataFrame in parallel.
...
def seq_apply_df_rows(df, func, use_tqdm: bool = True):
    """Apply a function to each row of a DataFrame in sequence, returning multiple Series.
    ...
    Returns:
            tuple[pd.Series, ...]: The results of applying the function to each row.
    """
    df_iter = (
        tqdm(df.iterrows(), desc=pformat(func), total=df.shape[0]) if use_tqdm else df.iterrows()
    )
    ...
    if isinstance(result_list[0], tuple):
        raise NotImplementedError("Use seq_apply_df_rows_t for multiple return values")
    result_series = pd.Series(result_list, index=df.index)
    return result_series


def seq_apply_df_rows_t(df, func, use_tqdm: bool = True) -> tuple[pd.Series, ...]:
    """Apply a function to each row of a DataFrame in sequence, returning multiple Series."""
    df_iter = (
        tqdm(df.iterrows(), desc=pformat(func), total=df.shape[0]) if use_tqdm else df.iterrows()
    )
...
def parse_inst_info(df):
    ...
    if none_or_nan(inst_type_str):
        ...
        inst_map = {"Maxis II HD Q-TOF Bruker": "QTOF", "qToF": "QTOF", "Orbitrap": "FT"}
        ...
        if inst_type_str in inst_type_map:
            ...
            inst_type = "Other"
    # fragmentation mode
    if inst_type_str == "HCD" or type(col_energy_str) == str and "HCD" in col_energy_str:
        frag_mode = "HCD"
...
def parse_annotations(row):
    ...
    notes = row["notes"]
    error = None
    current_supported_dsets = ["nist20_hr"]
    if dset not in current_supported_dsets:
        error = ValueError(
            f"dataset not supported, supported {current_supported_dsets}, given {dset}"
        )
...
def get_murcko_scaffold(mol, output_type="smiles", include_chirality=False) -> str | float:
    """get murcko scaffold from mol object
    Args:
            mol (rdkit.Chem.rdchem.Mol): mol object
            output_type (str, optional): output type. Defaults to "smiles".
            include_chirality (bool, optional): include chirality. Defaults to False.
    Returns:
            str: murcko scaffold
    """
    ...
    if output_type == "smiles":
        try:
            ...
            warnings.warn(f"failed to get murcko scaffold: {e}")
            scaffold = np.nan
    else:
...
def compute_mol_df_props(
    mol_df: "pd.DataFrame",
    scaffold: bool = True,
    inchi: bool = True,
    use_tqdm: bool = True,
) -> "pd.DataFrame":
    """Compute standard molecular properties and add them as columns to ``mol_df``.

    Requires a ``mol`` column containing RDKit mol objects. All new columns are
    written in-place via ``loc`` and the modified dataframe is also returned.

    The following columns are always added:
    ``inchikey_s``, ``formula``, ``mw``, ``exact_mw``, ``num_atoms``,
    ``num_bonds``, ``charge``, ``single_mol``, ``num_radicals``.

    Args:
        mol_df: DataFrame with a ``mol`` column of RDKit mol objects.
        scaffold: If ``True`` (default), also compute the ``scaffold`` column
            (Murcko scaffold SMILES).
        inchi: If ``True`` (default), also compute the ``inchi`` column.
        use_tqdm: Passed through to :func:`par_apply_series`; set ``False`` to
            suppress progress bars (e.g. in tight loops).

    Returns:
        The same ``mol_df`` with new property columns added in-place.
    """
    mol_col = mol_df["mol"]
    mol_df.loc[:, "inchikey_s"] = par_apply_series(mol_col, mol_to_inchikey_s, use_tqdm=use_tqdm)
    if scaffold:
        mol_df.loc[:, "scaffold"] = par_apply_series(
            mol_col, get_murcko_scaffold, use_tqdm=use_tqdm
        )
    mol_df.loc[:, "formula"] = par_apply_series(mol_col, mol_to_formula, use_tqdm=use_tqdm)
    if inchi:
        mol_df.loc[:, "inchi"] = par_apply_series(mol_col, mol_to_inchi, use_tqdm=use_tqdm)
    mol_df.loc[:, "mw"] = par_apply_series(
        mol_col, lambda mol: mol_to_mol_weight(mol, exact=False), use_tqdm=use_tqdm
    )
    mol_df.loc[:, "exact_mw"] = par_apply_series(
        mol_col, lambda mol: mol_to_mol_weight(mol, exact=True), use_tqdm=use_tqdm
    )
    mol_df.loc[:, "num_atoms"] = par_apply_series(mol_col, mol_to_num_atoms, use_tqdm=use_tqdm)
    mol_df.loc[:, "num_bonds"] = par_apply_series(mol_col, mol_to_num_bonds, use_tqdm=use_tqdm)
    mol_df.loc[:, "charge"] = par_apply_series(mol_col, mol_to_charge, use_tqdm=use_tqdm)
    mol_df.loc[:, "single_mol"] = par_apply_series(mol_col, check_single_mol, use_tqdm=use_tqdm)
    mol_df.loc[:, "num_radicals"] = par_apply_series(
        mol_col, mol_to_num_radicals, use_tqdm=use_tqdm
    )
    return mol_df


def analyze_mol(mol: "Chem.rdchem.Mol") -> dict:
    """get some basic properties of the mol object
    Args:
            mol (rdkit.Chem.rdchem.Mol): mol object
    Returns:
...
CHARGE_FACTOR_MAP = {1: 1.00, 2: 0.90, 3: 0.85, 4: 0.80, 5: 0.75, "large": 0.75}


def get_charge(prec_type_str: str) -> int:
    """
    Parse the charge from the precursor type string.
    Args:
            prec_type_str (str): Precursor type string.
    Returns:
            int: Charge of the precursor.
    """
    if prec_type_str == "EI":
        return 1
...
def nce_to_ace_helper(nce: float, charge: int, prec_mz: float) -> float:
    """Convert NCE to ACE.

    Args:
            nce (float): The normalized collision energy.
            charge (int): The charge state of the ion.
            prec_mz (float): The precursor mass-to-charge ratio.

    Returns:
            float: The calculated absolute collision energy.
    """
...
def ace_to_nce_helper(ace: float, charge: int, prec_mz: float) -> float:
    """Convert ACE to NCE.

    Args:
            ace (float): The absolute collision energy.
            charge (int): The charge state of the ion.
            prec_mz (float): The precursor mass-to-charge ratio.

    Returns:
            float: The normalized collision energy.
    """
    if charge in CHARGE_FACTOR_MAP:
        charge_factor = CHARGE_FACTOR_MAP[charge]
    else:
...
def nce_to_ace(row, nce_colname: str) -> float:
    """convert nce to ace
    Args:
            row (pd.Series): row of the dataframe
            nce_colname (str): column name of nce
    Returns:
            float: ace
    """
    prec_mz = row["prec_mz"]
...
def ace_to_nce(row, ace_colname: str) -> float:
    """convert ace to nce
    Args:
            row (pd.Series): row of the dataframe
            ace_colname (str): column name of ace
    Returns:
            float: nce
    """
    prec_mz = row["prec_mz"]
...
def fill_missing_nce(row, ace_colname: str, nce_colname: str) -> float:
    """fill missing nce with ace
    Args:
            row (pd.Series): row of the dataframe
    Returns:
            float: nce
    ...
    nce = row[nce_colname]
    ace = row[ace_colname]
    nce_missing = none_or_nan(nce) or nce == -1
    ace_missing = none_or_nan(ace) or ace == -1
    if nce_missing and not ace_missing:
        return ace_to_nce(row, ace_colname)
    elif not nce_missing:
        return nce
    else:
        warnings.warn(
            f"Warning: can not compute nce from ace. nce: {nce}, ace: {ace} from columns {nce_colname}, {ace_colname}"
        )
        return np.nan


def fill_missing_ace(row, ace_colname: str, nce_colname: str) -> float:
    """fill missing ace with nce
    Args:
            row (pd.Series): row of the dataframe
    Returns:
            float: ace
    ...
    ace_missing = none_or_nan(ace) or ace == -1
    nce_missing = none_or_nan(nce) or nce == -1
    if ace_missing and not nce_missing:
        return nce_to_ace(row, nce_colname)
    elif not ace_missing:
        return ace
    else:
        warnings.warn(
            f"Warning: can not compute ace from nce. ace: {ace}, nce: {nce} from columns {ace_colname}, {nce_colname}"
        )
        return np.nan
...
def deep_copy_data_dict(data_dict):
    """Deep copy data dict, used to avoid modifying the original dict.

    Handles key-tensor pairs and key-ROMol pairs specifically.

    Args:
            data_dict (dict): Data dict to copy.

    Returns:
            dict: Deep copied dict.
    """
    copied_dict = {}
    for key, value in data_dict.items():
        if isinstance(value, th.Tensor):
            # Use .clone() for tensors to create a deep copy
            # make sure tensors are detached to from computation graph
            copied_dict[key] = value.detach().clone()
        elif isinstance(value, dict):
            copied_dict[key] = deep_copy_data_dict(value)
        else:
            # Use deepcopy for other types
...
def min_max_normalize(value, min, max):
    ...
    return (value - min) / (max - min + 1e-8)


def min_max_denormalize(normalized_value, min, max):
    """Denormalize float predictions back to original scale."""
    return normalized_value * (max - min + 1e-8) + min


def zscore_normalize(value, mean, std):
    """Normalize float values using z-score normalization."""
    return (value - mean) / (std + 1e-8)


def zscore_denormalize(normalized_value, mean, std):
    """Denormalize float predictions back to original scale."""
    return normalized_value * (std + 1e-8) + mean
